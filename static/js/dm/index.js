(function() {
    ...
    function makeDmUserItem(item) {
        ...
        div.className = 'pointer d-flex flex-row w-full p-2 align-items-center'
        div.innerHTML = `
            <div class="w50">
                <img src="/static/img/profile/${item.opponent.iuser}/${item.opponent.mainimg}" class="circleimg profile w30 h30" 
                    onerror="this.onerror=null; this.src='/static/img/profile/user.png'">
            </div>
        ...
        div.addEventListener('click', function() {
            ...
            dmMsgContainerElem.classList.add('border');           
            isFirst = true;
            msgParam.page = 0;
            ...
            div2.forEach(clickbg => {
                if(clickbg.classList.contains('dmbgcolor')) {
                    clickbg.classList.remove('dmbgcolor');
                }
            });
            div.classList.add('dmbgcolor');
        });
...
function makeDmMsgItem(loginiuser, item) {
    ...
    const div = document.createElement('div');
    div.className = loginiuser === item.iuser ? 'ms-auto col-5 m-2': 'col-5 m-2';
